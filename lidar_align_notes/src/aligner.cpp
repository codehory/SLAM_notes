 ...
 * @brief 最近邻误差计算
 * @param kdtree
 * @param pointcloud 输入点云，为分组后的点云
 * @param k 最近邻个数=2
 * @param max_dist 最大距离阈值
 * @param start_idx 分组后对于全局点云的索引起点
 * @param end_idx 分组后对于全局点云的索引终点
 * @return
 */
...
/*!
 * @brief 处理Lidar数据，首先根据估计的Lidar到Odom的变化，对完整的scans_数据计算出每次scan时每个点在Odom下的坐标（getTimeAlignedPointcloud函数，相当于点云去畸变），得到一个结合的点云(CombinedPointcloud)
 * @param lidar
 * @return
 */
...
/*!
 * @brief 设置NLOPT优化的相关变量
 * @param lb 待优化变量的下界
 * @param ub 待优化变量的上界
 * @param opt_data 外部传入数据
 * @param x 待优化变量
 */
    void Aligner::optimize(const std::vector<double> &lb,
        ...
        if (config_.local) {
            ...
            /// 全局优化 只优化三个角度 选用全局优化算法DIRECT_L，不需要求导
