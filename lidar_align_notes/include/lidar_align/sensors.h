...
class OdomTformData {
 ...
 private:
  Transform T_o0_ot_; ///< 当前时刻里程计坐标系相对于起始时刻里程计的变换
  Timestamp timestamp_us_;
};

/*!
 * @brief 里程计类，用于保存全局INS位姿
 */
class Odom {
 public:
 ...
 private:
  std::vector<OdomTformData> data_; ///< 用于保存全局里程计
};
...
class Scan {
 ...
 private:
  ...
  std::vector<Transform>
      T_o0_ot_;  ///< T_o0_ot_中的元素：每个点根据时间戳从odom插值获得的变换矩阵 absolute odom transform to each point in pointcloud

  bool odom_transform_set_;
};

/*!
 * @brief 激光雷达类，用于保存所有时刻点云
 */
class Lidar {
 public:
 ...
 private:
  LidarId lidar_id_;
  Transform T_o_l_;  ///< 待优化量: 激光雷达到车体的变换矩阵 transform from lidar to odometry
