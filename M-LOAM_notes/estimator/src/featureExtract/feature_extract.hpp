                                         ...
                                         std::vector<PointPlaneFeature> &features)
{
    ...
    for (size_t i = 0; i < cloud_data.points.size(); i++)
    {
        ...
        if (point_search_sqdis[0] < DISTANCE_SQ_THRESHOLD)
        {
            ...
            ///分别在近邻点的线束的两个方向(增加/减少)找不在同一线束上距离当前点最近的点l(找到的点的线束不能与近邻点相同)
...
/*!
 * @brief 找surf三个近邻点，三个点用于计算点到面的距离   https://zhuanlan.zhihu.com/p/396331443
 * @tparam PointType
 * @param kdtree_surf_from_scan
 * @param cloud_scan
 * @param cloud_data
 * @param pose_local
 * @param features
                                       ...
                                       std::vector<PointPlaneFeature> &features)
{
    ...
    for (size_t i = 0; i < cloud_data.points.size(); i++)
    {
        ...
        if (point_search_sqdis[0] < DISTANCE_SQ_THRESHOLD)
        {
            ...
            ///找到在上一帧中j附近线束上的近邻点l和与j同一线束的近邻点m (近邻点: 当前点转换到上一帧坐标系下距离最近的点)
            // search in the direction of increasing scan line
            for (int j = closest_point_ind + 1; j < (int)cloud_scan.points.size(); j++)
            {
                // if not in nearby scans, end the loop
